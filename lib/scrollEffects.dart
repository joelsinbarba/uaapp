...
class EventDetailsScrollEffects {
  ...
  void _recalculateValues(BuildContext context) {
    ...
    if (unconstrainedBackdropHeight < kHeaderHeight)
      unconstrainedBackdropHeight = kHeaderHeight;
    ...
    headerOffset = _calculateHeaderOffset(kHeaderHeight + (-_scrollOffset));
  ...
  double _calculateBackdropBlur() {
    final backdropOverscrollBlur = max(0.0, min(20.0, -_scrollOffset / 22));
  ...
  double _calculateHeaderOffset(double unconstrainedBackdropHeight) {
    if (unconstrainedBackdropHeight < 300.0) {
      return -(300.0 - unconstrainedBackdropHeight);
    }
